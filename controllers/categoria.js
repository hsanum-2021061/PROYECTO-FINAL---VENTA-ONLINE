...
const getProductosCat = async (req = request, res = response) => {
    const { id } = req.params;

    //condiciones del get
    const query = { disponible: true,categoria:id };
    const listaCategoriaP = await Promise.all([
        Producto.countDocuments(query),
        Producto.find(query)
    ]);

    res.json({
        msg: 'Controlador Categoria',
        listaCategoriaP
    });
